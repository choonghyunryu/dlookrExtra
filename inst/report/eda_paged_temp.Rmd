...
value <- c("$dataset$", 
           class(reportData)[1], 
           paste0(N_sample, " / ", format(N, big.mark = ","), " (",
                  sample_percent, "%)"),
           "$date$",
...
# cap <- "Descriptive Statistics of Numerical Variables"
html_paged_describe_detail(reportData)
```
...
html_paged_categorical_detail(reportData)
```
...
html_paged_normality_detail(reportData)
```
...
html_paged_compare_numerical(reportData, caption = cap, font_size = 13)
```
...
html_paged_compare_categorical(reportData, caption = cap, font_size = 13)
```
...
```{r plot-correlation, results='asis', dpi=300, fig.height=5, fig.width=5, out.width="85%"}
...
```{r group-numerical, results='asis', dpi=400, fig.height=5, fig.width=7, out.width="85%"}
html_paged_target_numerical(reportData, targetVariable)  
```

## Grouped Categorical Variables

```{r group-categorical, results='asis', dpi=400, fig.height=6, fig.width=7, out.width="85%"}
html_paged_target_categorical(reportData, targetVariable)  
```

## Grouped Correlation

```{r group-correlation, results='asis', dpi=400, fig.height=6, fig.width=6, out.width="95%"}
html_paged_target_correlation(reportData, targetVariable)  
```
