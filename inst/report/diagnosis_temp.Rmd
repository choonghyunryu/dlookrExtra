...
thres_uniq_cat <- get("thres_uniq_cat", .dlookrExtraEnv)
thres_uniq_num <- get("thres_uniq_num", .dlookrExtraEnv)
```
...
overview <- data.frame(division = division, metrics = metrics, value = value)
```
...
html_variable(reportData, theme = "$theme$")
...
h2("Categorical Vaiables", id = "ID-h2-uniq-categorical", class = "title-h2")
```
...
h2("Numarical Vaiables", id = "ID-h2-uniq-numerical", class = "title-h2")
```
...
if (NROW(diagn_numeric) > 0) {
  ...
  if (NROW(diagn_outlier) > 0) {
    ...
    html_outlier(reportData, theme = "$theme$")
